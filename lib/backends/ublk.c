...
#define UBLKSRV_IO_ASYNC_PENDING	(1U << 4)
...
static volatile int g_sig_dev_id = -1;
...
{
	if (dev_id < 0 || !buf || buflen == 0)
		return -EINVAL;

	snprintf(buf, buflen, UBLK_BDEV_FMT, dev_id);
	return 0;
}
